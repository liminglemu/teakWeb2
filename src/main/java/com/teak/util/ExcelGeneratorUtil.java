...
/**
 * Excel生成工具类
 * 使用工厂模式和模板方法模式设计，便于扩展不同类型的Excel报表生成
 */
@Component
@Slf4j
public class ExcelGeneratorUtil {
    ...
    public Workbook generateProgressReportWorkbook(List<GenerateProgressReportResultDto> dataList) {
        ...
        // 获取每日列数
        int dailyColumnCount = getDailyColumnCount(dataList);
        if (dailyColumnCount <= 0) {
            log.error("数据为空,默认使用10列");
            dailyColumnCount = 10;
        }

        // 创建表头
        createHeader(sheet, dailyColumnCount);
        // 填充数据
        fillData(sheet, dataList, dailyColumnCount);
        // 冻结前4列A-D列
        sheet.createFreezePane(4, 0);
        // 设置列宽
        setColumnWidths(sheet, dailyColumnCount);
     ...
     * @return 测试数据列表
     */
    public List<GenerateProgressReportResultDto> createTestData() {
        // 7线-安规测试
    ...
    private void createHeader(XSSFSheet sheet, int dailyColumnCount) {
        ...
        sheet.addMergedRegion(new CellRangeAddress(0, 0, 4, 4 + dailyColumnCount - 1));

        // E2 开始：天数编号
        for (int i = 0; i < dailyColumnCount; i++) {
    ...
    private void fillData(XSSFSheet sheet, List<GenerateProgressReportResultDto> dataList, int dailyColumnCount) {
        // 从第3行开始写数据
        int currentRow = 2;
        ...
        for (GenerateProgressReportResultDto container : dataList) {
            ...
            int mergeEndRow = currentRow + 3 * resultList.size() - 1;
            ...
            sheet.addMergedRegion(new CellRangeAddress(currentRow, mergeEndRow, 0, 0));

            // 填充每个工序
            for (ProductionPlanMaintenanceQueryReportDto item : resultList) {
                fillSingleRow(sheet, item, currentRow, dailyColumnCount);
                currentRow += 3;
            }
        }
    }
    ...
    private void fillSingleRow(XSSFSheet sheet, ProductionPlanMaintenanceQueryReportDto item, int rowStart, int dailyColumnCount) {
        ...
        // E列：每日数据
        fillDailyDate(planRow, item.getPlanList(), dailyColumnCount, numberStyle);
        fillDailyDate(actualRow, item.getActualList(), dailyColumnCount, numberStyle);
        fillDailyDate(rateRow, item.getAchievementRateList(), dailyColumnCount, numberStyle);
    }

    private void fillDailyDate(XSSFRow row, List<? extends Number> dataList, int dailyColumnCount, CellStyle numberStyle) {
        if (CollectionUtil.isEmpty(dataList)) {
            // 填充空值占位
            for (int i = 0; i < dailyColumnCount; i++) {
    ...
    private void setColumnWidths(XSSFSheet sheet, int dailyColumnCount) {
